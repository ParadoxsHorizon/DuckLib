...
#   define PREDEF_WORKAROUND(symbol, comp, major, minor, patch) (0)
#else
#   include <boost/predef/version_number.h>
#   define BOOST_PREDEF_WORKAROUND(symbol, comp, major, minor, patch) \
        ( (symbol) != (0) ) && \
        ( (symbol) comp (BOOST_VERSION_NUMBER( (major) , (minor) , (patch) )) )
#   define PREDEF_WORKAROUND(symbol, comp, major, minor, patch) \
        ( (symbol) != (0) ) && \
        ( (symbol) comp (VERSION_NUMBER( (major) , (minor) , (patch) )) )
#endif
...
#   define PREDEF_TESTED_AT(symbol, major, minor, patch) (0)
#else
...
#       define BOOST_PREDEF_TESTED_AT(symbol, major, minor, patch) ( \
            ...
            ? 1 \
            : (1%0) )
#       define PREDEF_TESTED_AT(symbol, major, minor, patch) ( \
            PREDEF_WORKAROUND(symbol, <=, major, minor, patch) \
            ? 1 \
            : (1%0) )
#   else
#       define BOOST_PREDEF_TESTED_AT(symbol, major, minor, patch) \
...
#       define PREDEF_TESTED_AT(symbol, major, minor, patch) \
            ( (symbol) >= VERSION_NUMBER_AVAILABLE )
#   endif
#endif
